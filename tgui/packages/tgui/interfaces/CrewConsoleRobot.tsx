...
const HealthStat = (props: HealthStatProps) => {
  ...
  return (
    <Box inline width={5} color={COLORS.damageType[type]} textAlign="center">
      {value + ' / 100'}
    </Box>
  );
};

export const CrewConsoleRobot = () => {
  return (
    <Window title="Robot Distress Beacon Monitor" width={600} height={600}>
      ...
      <Table.Cell collapsing textAlign="center">
        {power !== undefined ? (
          <Icon
            ...
            color={healthToAttribute(power, oil, HEALTH_COLOR_BY_LEVEL)}
            size={1}
          />
        ) : life_status !== STAT_DEAD ? (
          <Icon name="plug" color="#17d568" size={1} />
        ) : (
          <Icon name="plug-circle-bolt" color="#801308" size={1} />
        )}
      </Table.Cell>
      <Table.Cell collapsing textAlign="center">
        {power !== undefined ? (
