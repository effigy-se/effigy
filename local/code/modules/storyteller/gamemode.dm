SUBSYSTEM_DEF(gamemode)
	...
	/// Result of the storyteller vote
	...
	var/list/point_thresholds = list(
		...
		EVENT_TRACK_CREW = 100,
		EVENT_TRACK_GHOST = 100
		)
	...
	var/list/min_pop_thresholds = list(
		...
		EVENT_TRACK_CREW = CREWSET_MIN_POP,
		EVENT_TRACK_GHOST = GHOSTSET_MIN_POP
		)
	...
	var/list/pop_scale_thresholds = list(
		...
		EVENT_TRACK_CREW = CREWSET_POP_SCALE_THRESHOLD,
		EVENT_TRACK_GHOST = GHOSTSET_POP_SCALE_THRESHOLD
		)
	...
	var/list/pop_scale_penalties = list(
		...
		EVENT_TRACK_CREW = CREWSET_POP_SCALE_PENALTY,
		EVENT_TRACK_GHOST = GHOSTSET_POP_SCALE_PENALTY
		)
	...
	/// Amount of initiated extension votes
	var/extensions_voted = 0
		...
		if(event.holidayID)
			...
			for(var/datum/holiday/current_holiday as anything in GLOB.holidays)
				if(current_holiday == event.holidayID)
					log_game("Event [event] enabled due to active holiday [current_holiday]")
					message_admins("Event [event] enabled due to active holiday [current_holiday].")
...
/datum/controller/subsystem/gamemode/proc/roll_pre_setup_points()
	...
	if(storyteller.guarantees_roundstart_crewset)
		event_track_points[EVENT_TRACK_CREW] = point_thresholds[EVENT_TRACK_CREW]
...
/datum/controller/subsystem/gamemode/proc/load_config_vars()
	...
	point_gain_multipliers[EVENT_TRACK_CREW] = CONFIG_GET(number/crewset_point_gain_multiplier)
	point_gain_multipliers[EVENT_TRACK_GHOST] = CONFIG_GET(number/ghostset_point_gain_multiplier)
	...
	roundstart_point_multipliers[EVENT_TRACK_CREW] = CONFIG_GET(number/crewset_roundstart_point_multiplier)
	roundstart_point_multipliers[EVENT_TRACK_GHOST] = CONFIG_GET(number/ghostset_roundstart_point_multiplier)
	...
	min_pop_thresholds[EVENT_TRACK_CREW] = CONFIG_GET(number/crewset_min_pop)
	min_pop_thresholds[EVENT_TRACK_GHOST] = CONFIG_GET(number/ghostset_min_pop)
...
/datum/controller/subsystem/gamemode/proc/init_storyteller()
	if(storyteller) // If this is true, then an admin bussed one, don't overwrite it
		log_dynamic("Roundstart game mode has been set by admins to [storyteller.name], the vote was not considered.")
		return
	var/datum/storyteller/storyteller_pick
	if(!voted_storyteller)
		storyteller_pick = pick(storytellers)
		log_dynamic("Roundstart picked game mode [storyteller.name] randomly due to no vote result.")
...
/datum/controller/subsystem/gamemode/proc/set_storyteller(passed_type, forced, force_ckey)
	if(!storytellers[passed_type])
		message_admins("Attempted to set an invalid game mode type: [passed_type].")
		CRASH("Attempted to set an invalid game mode type: [passed_type].")
...
/datum/controller/subsystem/gamemode/proc/halt_storyteller(mob/user)
	...
	if(isnull(user))
		return
	message_admins("[key_name_admin(user)] has [storyteller_halted ? "HALTED" : "un-halted"] game mode processing.")
	log_dynamic("Game mode processing [storyteller_halted ? "halted" : "un-halted"] by admin [user.ckey].")
...
/datum/controller/subsystem/gamemode/proc/force_next_event(track, mob/user)
	...
	log_admin_private("Game mode track [track] forced to run an event by [user.ckey]")
