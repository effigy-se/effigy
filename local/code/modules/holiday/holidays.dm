...
/datum/holiday/proc/queue_storyteller_celebration(datum/round_event/event, datum/round_event_control/control)
	if(isnull(event) || isnull(control))
		stack_trace("Invalid holiday event passed to game mode")
		return
...
/datum/holiday/effigy_launch
	name = "Effigy Anniversary"
	begin_day = 15
	end_day = 17
...
/datum/holiday/effigy_launch/greet()
	return "We celebrate this time of year when the current iteration of the Effigy project was launched! Humanity, in its infinite wisdom decided that what the cosmos lacked was a giant, floating, metal homage to our technological degeneracy. With blackjack, and hookers."
