...
/datum/sprite_accessory/clothing/underwear/make_appearance(color, physique, bodyshape)
	...
	if(physique == FEMALE && gender == MALE)
		underwear_overlay = mutable_appearance(wear_female_version(underwear_icon_state, icon, female_sprite_flags), layer = -UNDERWEAR_UNDERSHIRT)
	else
		underwear_overlay = mutable_appearance(icon, underwear_icon_state, -UNDERWEAR_UNDERSHIRT)
