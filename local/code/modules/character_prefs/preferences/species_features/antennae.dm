...
/datum/preference/toggle/antennae/is_accessible(datum/preferences/preferences)
	...
	if(!is_type_in_typecache(species.type, GLOB.bodypart_allowed_species[FEATURE_MOTH_ANTENNAE]))
		return FALSE
...
/datum/species/regenerate_organs(mob/living/carbon/target, datum/species/old_species, replace_current = TRUE, list/excluded_zones, visual_only = FALSE, replace_missing = TRUE)
	. = ..()
	if(target.dna.features[FEATURE_MOTH_ANTENNAE] && is_type_in_typecache(type, GLOB.bodypart_allowed_species[FEATURE_MOTH_ANTENNAE]))
