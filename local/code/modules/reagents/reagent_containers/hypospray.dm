...
#define ADV_WAIT_INJECT 1.5 SECONDS
#define ADV_WAIT_SPRAY 1 SECONDS
#define ADV_SELF_INJECT 1 SECONDS
#define ADV_SELF_SPRAY 1 SECONDS
...
/obj/item/hypospray/mkii
	...
	icon_state = "hypo2"
	icon = 'local/icons/obj/medical/hypospray/hyposprays.dmi'
...
/obj/item/hypospray/mkii/piercing
	...
	inject_wait = ADV_WAIT_INJECT
	spray_wait = ADV_WAIT_SPRAY
	spray_self = ADV_SELF_INJECT
	inject_self = ADV_SELF_SPRAY
...
/obj/item/hypospray/mkii/proc/do_inject(mob/living/injectee, mob/living/user, mode)
	...
	playsound(loc, long_sound ? 'local/sound/items/hypospray/hypospray_long.ogg' : pick('local/sound/items/hypospray/hypospray.ogg','local/sound/items/hypospray/hypospray2.ogg'), 50, 1, -1)
...
#undef SELF_INJECT
#undef SELF_SPRAY
#undef ADV_WAIT_INJECT
#undef ADV_WAIT_SPRAY
#undef ADV_SELF_INJECT
#undef ADV_SELF_SPRAY
...
/obj/item/reagent_containers/cup/hypovial
	...
	icon = 'local/icons/obj/medical/hypospray/hypovials.dmi'
	icon_state = "hypovial"
	...
	var/type_suffix = "-s"
	/// For determining atom_skins used
	var/large = FALSE
	fill_icon = 'local/icons/obj/medical/hypospray/hypospray_fillings.dmi'

/obj/item/reagent_containers/cup/hypovial/Initialize(mapload)
	. = ..()
	AddComponent(/datum/component/reskinable_item, large ? /datum/atom_skin/hypovial_large : /datum/atom_skin/hypovial)
...
/datum/atom_skin/hypovial/custom/apply(atom/apply_to)
	. = ..()
	var/list/allowed_configs = list("[apply_to::greyscale_config]")
	apply_to.greyscale_colors ||= "#FFFF00"
	var/datum/greyscale_modify_menu/menu = new(apply_to, usr, allowed_configs)
	menu.ui_interact(usr)

/datum/atom_skin/hypovial/custom/clear_skin(atom/clear_from)
	. = ..()
	clear_from.greyscale_colors = null
...
/obj/item/reagent_containers/cup/hypovial/large
	...
	greyscale_config = /datum/greyscale_config/hypovial_large
...
/datum/atom_skin/hypovial_large/custom/apply(atom/apply_to)
	. = ..()
	var/list/allowed_configs = list("[apply_to::greyscale_config]")
	apply_to.greyscale_colors ||= "#FFFF00"
	var/datum/greyscale_modify_menu/menu = new(apply_to, usr, allowed_configs)
...
/datum/atom_skin/hypovial_large/custom/clear_skin(atom/clear_from)
	. = ..()
	clear_from.greyscale_colors = null
