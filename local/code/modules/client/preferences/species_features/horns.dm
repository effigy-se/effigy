...
/datum/bodypart_overlay/mutant/horns/override_color(obj/item/bodypart/bodypart_owner)
	...
	. = bodypart_owner.owner.dna.features["horns_color_1"]
	draw_color_2 = bodypart_owner.owner.dna.features["horns_color_2"]
	draw_color_3 = bodypart_owner.owner.dna.features["horns_color_3"]

/datum/bodypart_overlay/mutant/horns/color_image(image/overlay, obj/item/bodypart/limb, layer_index)
	if(limb == null)
		return ..()
	if(limb.owner == null)
		return ..()
	switch(layer_index)
		if(EXTERNAL_FRONT, EXTERNAL_ADJACENT, EXTERNAL_BEHIND)
			...
			return overlay
		if(EXTERNAL_FRONT_2, EXTERNAL_ADJACENT_2, EXTERNAL_BEHIND_2)
			overlay.color = draw_color_2
			return overlay
		if(EXTERNAL_FRONT_3, EXTERNAL_ADJACENT_3, EXTERNAL_BEHIND_3)
