...
/obj/machinery/dna_fixer
	...
	"The DNA Fixer is not to be used un-cleaned. Thanks to its non-stick coating, cleaning up after a failed rejuvenation is easy as cleaning a microwave. Blood just doesn't stick!", \
	"Before using the DNA Fixer, remove any and all metal devices, or you might make the term 'ironman' a bit too literal!" , \
	"Remember, this is not cloning! Self-Actualization is a legally distinct, Veymed patent pending procedure. Still have questions? Call your nearest Veymed Representative to requisition more information about the DNA Fixer!" , \
	)
	COOLDOWN_DECLARE(advert_time)
	COOLDOWN_DECLARE(sad_processing_time)

/obj/machinery/dna_fixer/examine_more(mob/user)
	. = ..()
...
/obj/machinery/dna_fixer/Initialize(mapload)
	. = ..()
	sound_loop = new(src, FALSE)
	register_context()
	update_appearance()

/obj/machinery/dna_fixer/Destroy()
...
/obj/machinery/dna_fixer/close_machine(atom/movable/target, density_to_set = TRUE)
	..()
...
/obj/machinery/dna_fixer/examine(mob/user)
	. = ..()
...
/obj/machinery/dna_fixer/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	if(!processing)
...
/obj/machinery/dna_fixer/interact(mob/user)
	if(state_open)
		close_machine()
		return
...
/obj/machinery/dna_fixer/process(seconds_per_tick)
	if(!processing)
		return
...
/obj/machinery/dna_fixer/proc/get_consent()
...
/obj/machinery/dna_fixer/proc/eject_new_you()
	...
	log_game("[key_name(patient)] used a DNA Fixer at [loc_name(src)].")

	if(patient.dna.real_name != original_name)
		message_admins("[key_name_admin(patient)] has used the DNA Fixer, and changed the name of their character. \
...
/obj/machinery/dna_fixer/proc/eject_old_you(damaged_goods = FALSE)
	player_consent = NO_CONSENT
	set_light(l_on = FALSE)
	sound_loop.stop()
	processing = FALSE
...
/obj/machinery/dna_fixer/crowbar_act(mob/living/user, obj/item/used_item)
	if(occupant)
		to_chat(user, span_warning("[src] is currently occupied!"))
		return
...
/obj/machinery/dna_fixer/RefreshParts()
	. = ..()
