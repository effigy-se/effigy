...
/datum/reagent/consumable/ethanol/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, metabolization_ratio)
	...
	if(drinker.get_drunk_amount() < volume * boozepwr * ALCOHOL_THRESHOLD_MODIFIER || boozepwr < 0)
		...
		drinker.adjust_drunk_effect(1 * booze_power * ALCOHOL_RATE * metabolization_ratio * seconds_per_tick) // EffigyEdit Change - Alcohol Processing - Original: (1 * sqrt(volume) * booze_power * ALCOHOL_RATE * metabolization_ratio * seconds_per_tick)
		if(boozepwr > 0)
