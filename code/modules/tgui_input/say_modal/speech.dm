...
/datum/tgui_say/proc/alter_entry(payload)
	...
	/// No OOC leaks
	//if(!entry || payload["channel"] == OOC_CHANNEL || payload["channel"] == ME_CHANNEL || payload["channel"] == PRAY_CHANNEL) // EffigyEdit Change - LOOC
	if(!entry || payload["channel"] == OOC_CHANNEL || payload["channel"] == ME_CHANNEL || payload["channel"] == PRAY_CHANNEL || payload["channel"] == LOOC_CHANNEL)
		return pick(phrases)
