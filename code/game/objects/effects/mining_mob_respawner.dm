...
/obj/effect/mining_mob_respawner
	...
	/// Are we waiting for a mob to spawn so we can link to it? // EffigyEdit Add - Early pull TG #96827
	var/registered_spawn_signal = FALSE // EffigyEdit Add - Early pull TG #96827
...
/obj/effect/mining_mob_respawner/Initialize(mapload)
	...
	if (!our_mob?.resolve() && !registered_spawn_signal) // EffigyEdit Change - Early pull TG #96827
		make_mob()
...
/obj/effect/mining_mob_respawner/proc/setup(list/valid_mobs, atom/initial_spawn)
	...
	if (istype(initial_spawn, /obj/effect/spawner/random))
		...
		registered_spawn_signal = TRUE // EffigyEdit Add - Early pull TG #96827
	else
...
/obj/effect/mining_mob_respawner/proc/get_spawned_mob(datum/source, atom/new_spawn)
	...
	registered_spawn_signal = FALSE // EffigyEdit Add - Early pull TG #96827
