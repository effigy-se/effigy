...
/obj/structure/dresser/attack_hand(mob/user, list/modifiers)
	...
	var/choice = tgui_input_list(user, "Underwear, Bra, Undershirt or Socks?", "Changing", list("Underwear", "Underwear Color", "Bra", "Bra Color", "Undershirt", "Undershirt Color", "Socks", "Socks Color")) // EffigyEdit Change - Character Preferences - Original: tgui_input_list(user, "Underwear, Undershirt, or Socks?", "Changing", list("Underwear","Underwear Color","Undershirt","Socks"))
	if(isnull(choice))
		return
	...
	switch(choice)
		...
		if("Underwear Color")
			var/new_underwear_color = tgui_color_picker(dressing_human, "Choose your underwear color", "Underwear Color", dressing_human.underwear_color) // EffigyEdit Change - TGUI Color Picker
