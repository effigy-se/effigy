...
/datum/communciations_controller/proc/make_announcement(mob/living/user, is_silicon, input, syndicate, list/players)
	...
	if(is_silicon)
		...
		if(syndicate)
			...
			priority_announce(html_decode(message_data["message"]), null, 'sound/announcer/announcement/announce_dig.ogg', ANNOUNCEMENT_TYPE_CAPTAIN, has_important_message = TRUE, players = players) // EffigyEdit Change - Original: announce.ogg
